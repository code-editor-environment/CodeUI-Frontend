...
function RenderComment({ comment, onEditComment, onDelete, index }) {
  const dispatch = useDispatch();
  const [check, setCheck] = useState(false);
  const [checks, setChecks] = useState(false);
  const { profileRes } = useIsLogin();
  const editComment = async () => {
    await onEditComment(comment.id, checks);
    setChecks(false);
  }
  return (
    <>
              ...
              <div className="flex flex-col w-full gap-3">
                <div className="relative w-full bg-dark-600 flex items-center gap-4 focus-visible:ring-sky-400  rounded-xl overflow-hidden">
                  ...
                  <button
                    ...
                    onClick={editComment}
                    disabled={checks === comment.commentContent}
                  >
                    Send
            ...
            {profileRes?.username === comment.account.username && (
              <>
                <button
                  ...
                  onClick={() => setChecks(comment.commentContent)}
                >
                  Edit
                </button>
                <button
