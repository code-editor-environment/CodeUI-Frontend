...
function Profile() {
  ...
  useEffect(
    () => {
      getListElementByCreator({ login, page }).then((data) => {
        if (data.error) {
          console.log(data.error);
        } else {
      ...
      {!profiles ? (
        ...
        <>
          <section className="profile">
            ...
            />
            <aside>
              <h1 className="username">{profiles.name}</h1>
              <h4 className="profile-name">@{profiles.login}</h4>
              <div className="bio">{profiles.bio}</div>
